...
export const indodaxParser: Parser = (email: EmailEnvelope): ParsedReceipt | null => {
  ...
  if (depositIdr) {
    return {
      ...
      data: { amount: depositIdr[1] ?? null, asset: "IDR", currency: "IDR" },
      confidence: 0.95,
    };
  }
  ...
  if (depositCrypto) {
    return {
      ...
      data: { amount: depositCrypto[1] ?? null, asset: (depositCrypto[2] ?? "").toUpperCase() },
      confidence: 0.9,
    };
  }
  ...
  if (buy) {
    return {
      ...
      data: { side: "buy", amount: buy[1] ?? null, asset: (buy[2] ?? "").toUpperCase(), price: buy[3] ?? null, currency: "IDR" },
      confidence: 0.9,
    };
  }
  ...
  if (sell) {
    return {
      ...
      data: { side: "sell", amount: sell[1] ?? null, asset: (sell[2] ?? "").toUpperCase() },
      confidence: 0.9,
    };
  }
